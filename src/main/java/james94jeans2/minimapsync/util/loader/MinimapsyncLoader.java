...
public class MinimapsyncLoader implements IWaypointLoader {
	...
	public WaypointList loadWaypointsForDimension(int pDim) {
		...
		fileName.append(Minecraft.getMinecraft().func_147104_D().serverIP.replaceAll(":", "[colon]"));
		fileName.append("/DIM");
		...
		try {
			...
			returnList = new WaypointList(pDim, false);
			while (waypointReader.hasNextLine()) {
				returnList.add(new Waypoint(waypointReader.readNextLine(), pDim));
			}
		}catch (IOException e) {
		}finally {
	...
	public void saveWaypointsForDimension(WaypointList list) {
		...
		try {
			...
			for (Waypoint waypoint : list) {
				fw.write(waypoint.getNetworkLine() + "\n");
			}
		} catch (IOException e) {
		} finally {
			try {
